...
public class HotelController {
	...
	public String addHotelImpl(@ModelAttribute("hotel") Hotels hotel,
	...
	public String addRoom(@RequestParam("hotelId") int hotelId, Model model,
		...
		model.addAttribute("roomTypeList", new String[] { "Select",
				"Standard non A/C room", "Standard A/C room",
				"Executive A/C room", "Deluxe A/C room" });
		return "addRoom";
	}

	@RequestMapping(value = "/addRoomImpl.htm", method = RequestMethod.POST)
	public String addRoomImpl(@ModelAttribute("room") RoomDetails room,
			Model model, BindingResult result) {
	...
	public String modifyHotelImpl(@ModelAttribute("hotel") Hotels hotel,
			Model model, BindingResult result) {
	...
	public String modifyRoomImpl(@ModelAttribute("room") RoomDetails room,
			Model model, BindingResult result) {
	...
	public String viewBookingsOnDate(Model model)
	{
		...
		List<BookingDetails> list=hotelService.viewBookingDetailsFromDate(date);
		if(list.isEmpty())
			list=null;
	...
	public String viewBookingsOnDateSpecified(@RequestParam("date") Date date,Model model)
	{
		List<BookingDetails> list=hotelService.viewBookingDetailsFromDate(date);
		if(list.isEmpty())
			list=null;
	...
	public String viewBookingsOfHotel(@RequestParam("hotelId") int hotelId,Model model)
	{
		List<Hotels> list=hotelService.viewAllHotels();
		if(list.isEmpty())
			list=null;
		List<BookingDetails> list2=hotelService.viewBookingSpecificHotel(hotelId);
		if(list2.isEmpty())
		{
			list2=null;
			model.addAttribute("message", "No Room Booked in This Hotel");
		}model.addAttribute("hotelId", hotelId);
		model.addAttribute("list", list);
		model.addAttribute("list2", list2);
	...
	public String viewGuestList(@RequestParam("hotelId") int hotelId,Model model)
	{
		List<Hotels> list=hotelService.viewAllHotels();
		if(list.isEmpty())
			list=null;
		List<UserDetails> list2=hotelService.viewGuestListSpecificHotels(hotelId);
		model.addAttribute("list", list);
		model.addAttribute("list2", list2);
		model.addAttribute("hotelId", hotelId);
